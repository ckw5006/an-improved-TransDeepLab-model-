...
from model_12.backbones.swin import BasicLayer
from model_12.cross_attn import CBAMBlock,GAM_Attention
...
class SwinASPP(nn.Module):
    def __init__(self, input_size, input_dim, out_dim, cross_attn,
        ...
        if cross_attn == 'GAM':   #in_channels, out_channels,input_size
            self.proj = GAM_Attention(
                                  in_channels=len(self.layers)*input_dim,out_channels=out_dim, input_size=input_size)
        else:
    ...
    def forward(self, x):
        ...
        # print("features123",features.size())
